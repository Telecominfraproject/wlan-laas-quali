    ...
    try:
        ...
        if inc_lanforge == "Yes":
            ...
            for resource in sandbox.automation_api.FindResources(resourceFamily="CS_GenericResource", resourceModel="Trafficgenerator", \
                ...
                if resource.ReservedStatus != "Reserved":
                    sandbox.automation_api.AddResourcesToReservation(reservationId=sandbox.id, resourcesFullPath=[resource.Name], shared=False)
                    sandbox.automation_api.SetReservationResourcePosition(reservationId=sandbox.id,
        ...
        if namespace == '':
            for each in sandbox.automation_api.GetReservationDetails(sandbox.id).ReservationDescription.Services:
                if each.ServiceName == 'Helm Service V2':
                    ...
                    owrrm_version = InputNameValue(Name='owrrm_version', Value=sandbox.global_inputs['owsub Version'])
...
#identifies if the lab type is Basic / Advanced
...
# Checks if blueprint being used is the Generic InterOp BP or Specific Interop BP. Returns true if it is the latter


def is_specific_bp(sandbox):
    return "Specific" in sandbox.automation_api.GetReservationDetails(reservationId=sandbox.id).ReservationDescription.Name.split("-")[-1]


sandbox = Sandbox()
DefaultSetupWorkflow().register(sandbox)
if is_specific_bp(sandbox):
