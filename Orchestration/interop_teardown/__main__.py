...
global default_securityToken
default_perfectoURL = 'tip'



def get_reserved_phones(sandbox):
...
def phone_unreservation(sandbox, components):
    try:
        ...
        for each in sandbox.automation_api.GetReservationDetails(sandbox.id).ReservationDescription.Resources:
            if each.ResourceModelName == 'Phone':
                res = sandbox.automation_api.ExecuteCommand(sandbox.id, each.Name, 'Resource', 'perfecto_phone_unreserve',[perfectoURL_value,securityToken_value], printOutput=True)
                sandbox.automation_api.WriteMessageToReservationOutput(sandbox.id,"Perfecto Phone Unreservation Complete")
    except Exception as e:
        sandbox.automation_api.WriteMessageToReservationOutput(sandbox.id, "ERROR: Phone Unreservation Failed (check input values): " + e.message)
...
sandbox.workflow.before_teardown_started(phone_unreservation,[])
