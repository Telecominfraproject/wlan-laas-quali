...
class HelmServiceV2Driver (ResourceDriverInterface):
    ...
    def helm_install(self, context, chart_version, owgw_version, owsec_version, owfms_version, owgwui_version, owprov_version, owprovui_version, owanalytics_version, owsub_version, owrrm_version):
        ...
        api_session.WriteMessageToReservationOutput(res_id, "Downloading Helm Install Script: {}".format(service_resource.helm_deploy_script_url))
        ...
        git.Repo.clone_from(service_resource.helm_deploy_script_url, working_dir2, branch='main',depth=1)


        github_path = os.path.join(*(service_resource.github_repo_path_to_chart.split('/')))
        cert_path = os.path.join(working_dir2, github_path)
        ...
        os.environ['OWRRM_VERSION'] = owrrm_version
        os.environ['OWSUB_VERSION'] = owsub_version

        os.environ['VALUES_FILE_LOCATION'] = 'values.openwifi-qa.yaml'
    ...
    def helm_uninstall(self, context):
        ...
        if result.returncode != 0:
            if len(errors) > 0:
                api_session.WriteMessageToReservationOutput